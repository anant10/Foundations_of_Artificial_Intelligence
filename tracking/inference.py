...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while count < self.numParticles:
            for states in self.legalPositions:
                if count < self.numParticles:
                    self.particles.append(states)
                count += 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            distForBeliefs = self.getBeliefDistribution()
            for particle in distForBeliefs:
                distForBeliefs[particle] = distForBeliefs[particle] * emissionModel[util.manhattanDistance(pacmanPosition, particle)]

            if distForBeliefs.totalCount() == 0.0:
                self.initializeUniformly(gameState)
            else:
                distForBeliefs.normalize()
                self.particles = [util.sample(distForBeliefs) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(0, self.numParticles):
            newDistributionForPos = self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[i]))
            nextParticles.append(util.sample(newDistributionForPos))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        newPos = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(newPos)

        self.particles = []
        count = 0
        while count < self.numParticles:
            for states in newPos:
                if count < self.numParticles:
                    self.particles.append(states)
                count += 1
    ...
    def observeState(self, gameState):
        ...
        for eachParticle in self.particles:
            ...
            allPossible[eachParticle] += (emissionModels[0][dist1] * emissionModels[1][dist2])
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(numGhost):
                newGhostPos = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i,
                                                              self.ghostAgents[i])
                newParticle[i] = util.sample(newGhostPos)
